...
async function main() {
    ...
    const tx2 = await owner1.sendTransaction({
      to: "0x1CaFaAD105a4EfF01426A39dF1a535204ba1fCE4",
      value: ethers.constants.WeiPerEther.mul(5),
    });
    await tx2.wait();

     //  impersonating vitalik's account
     const vitalik_address = "0x1CaFaAD105a4EfF01426A39dF1a535204ba1fCE4";
    ...
    global.saltTable = {
      ...
      var impls =  {
       ...
       var data1 =   fs.readFileSync(
        ...
        var tx = await soul.smashEggs(50,"0x0000000000000000000000000000000000000000",{value:ethers.utils.parseUnits("0.05","ether")});
        await tx.wait();
      ...
      var num1 = await soul.typeAmountList(4);
        ...
        await soul.transferSynthesis(addr1.address,10);
        var num1 = await soul.refferAmount(owner.address);
        var num2 = await soul.refferAmount(addr1.address);
        console.log(num1,num2);
